...
*/
  ...
  public void robotInit() {
    ...
    _Climber.configSelectedFeedbackSensor(FeedbackDevice.CTRE_MagEncoder_Relative, 0, 30);
    _Climber.setSelectedSensorPosition(0, 0, 30);
    ...
    /*
    _FourBar.configContinuousCurrentLimit(2, 30);
    _FourBar.configPeakCurrentDuration(1000, 30);
    _FourBar.configPeakCurrentLimit(2, 30);
    */
  ...
  public void robotPeriodic() {
    ...
    /*if (_FourBar.getOutputCurrent() > 25){
    _FourBar.set(ControlMode.PercentOutput, 0);
    }*/
    SmartDashboard.putNumber("ClockTime", Clock.get());
    SmartDashboard.putBoolean("BackCylinders", BackCylinders);
    SmartDashboard.putNumber("ClimberArm", _Climber.getSelectedSensorPosition());
  ...
  public void autonomousPeriodic() {
    ...
    double[] xy = deadzone( _Driver.getRawAxis(4), _Driver.getRawAxis(1));
    drive(xy[0], xy[1]);
...
FourBarPresets(0, 0);
  ...
  public void ClimerPreset(){
    Status = "BallMiddle";
    while(_Driver.getRawButton(2) != true){
    if((FourBarEncoder.get() < 3900)){
      _Climber.setSelectedSensorPosition(0, 0, 30);
    }
    if((_Climber.getSelectedSensorPosition() < 1750)){
      _Climber.set(ControlMode.PercentOutput, 0.4);
    }
    if((_Climber.getSelectedSensorPosition() > 1750) && (FourBarEncoder.get() > 1)){
      Status = "InsideFrame";
      _Climber.set(ControlMode.PercentOutput, 0);
    }
    if(_FourBar.getSensorCollection().isRevLimitSwitchClosed()){
    ...
    if((FourBarEncoder.get() < 10) && (BackCylinders = true) && (_Climber.getSelectedSensorPosition() < 4000) && (_Climber.getOutputCurrent() < 15)){
      _Climber.set(ControlMode.PercentOutput, 0.9);
    }
    if(_Climber.getSelectedSensorPosition() > 4000){
      _Climber.set(ControlMode.PercentOutput, 0);
      FullRotation = true;
    }
    if((FullRotation = true) && (_RightMaster.getSelectedSensorPosition() < 2000) && (_LeftMaster.getSelectedSensorPosition() < 2000)){
      drive(0, -0.25);
    }
